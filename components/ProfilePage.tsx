...
const ProfilePage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchUserNotes = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const notesArray = notes.items || notes;
          setUserNotes(Array.isArray(notesArray) ? notesArray : []);
          setNotesError(null);
        } else {
          throw new Error(`API failed with status: ${response.status}`);
        }
      } catch (error) {
        console.warn('API failed, using mock data for user notes:', error);
        setNotesError('API bağlantısı başarısız, örnek veriler gösteriliyor');
        
        // Fallback to mock data - filter notes by current user
        try {
          const { mockNotes } = await import('../data/mockData');
          const userUploadedNotes = mockNotes.filter(note => 
            note.uploader === `${currentUser.firstName} ${currentUser.lastName}` ||
            note.uploader === currentUser.email ||
            // If no specific uploader match, show some sample notes for demo
            mockNotes.indexOf(note) < 3
          );
          setUserNotes(userUploadedNotes);
        } catch (mockError) {
          console.error('Failed to load mock data:', mockError);
          setUserNotes([]);
          setNotesError('Notlar yüklenirken hata oluştu');
        }
      } finally {
    ...
    // Only fetch once when component mounts or user ID changes
    fetchUserNotes();
  }, [currentUser?.id]);
      ...
      <div>
        ...
        {notesError && (
          <div className="mb-4 p-3 bg-yellow-100 dark:bg-yellow-900/20 border border-yellow-300 dark:border-yellow-700 rounded-lg">
            <p className="text-sm text-yellow-800 dark:text-yellow-200">{notesError}</p>
          </div>
        )}
        {loadingNotes ? (
